...
async def rdp_request_start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat = update.effective_chat

    if chat.type == ChatType.PRIVATE:
        await context.bot.send_message(
            chat_id=chat.id,
            text="Аккаунт 110202020⚙️",
            reply_markup=rdp_request_keyboard,
...
rdp_request_handler = ConversationHandler(
    entry_points=[
        MessageHandler(filters.Regex("Запросы по RDP") & ~filters.COMMAND, rdp_request_start_handler)
    ],
    states={
        CHOOSE_ACTION: [
            ...
            MessageHandler(filters.Regex("Назад") & ~filters.COMMAND, rdp_request_back_handler)
        ]
